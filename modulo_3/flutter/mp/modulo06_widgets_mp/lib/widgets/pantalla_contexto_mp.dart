  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          'Portal ${esMovil ? "Móvil (Cliente)" : "Web (Agencia)"} · ${esRetrato ? "Vertical" : "Horizontal"}',
      ...
      body: ListView(
        ...
        children: [
          ...
          _Seccion(
            titulo: 'Datos del Dispositivo (App de Rentas)',
            items: [
              'Ancho de vista:      ${tamanio.width.toStringAsFixed(0)} px',
              'Alto de vista:        ${tamanio.height.toStringAsFixed(0)} px',
              'Densidad de píxeles:  ${MediaQuery.devicePixelRatioOf(context).toStringAsFixed(3)}',
              'Espacio seguro sup.:  ${MediaQuery.paddingOf(context).top}',
            ],
          ),
          const SizedBox(height: 16),

          // ── Colores del tema ───────────────────────────────────
          _Seccion(titulo: 'Colores Corporativos de la Agencia', items: []),
          Wrap(
          ...
          // ── Tipografía del tema ────────────────────────────────
          _Seccion(titulo: 'Tipografías (Contratos y Catálogo)', items: []),
